...
SEC("tc_clash.meta_redirect_to_tun")
int tc_tun_func(struct __sk_buff *skb) {
    ...
    __u32 key = 0, *clash.meta_mark, *tun_ifindex;

    clash.meta_mark = bpf_map_lookup_elem(&tc_params_map, &key);
    if (!clash.meta_mark)
        return TC_ACT_OK;

    if (skb->mark == *clash.meta_mark)
        return TC_ACT_OK;
