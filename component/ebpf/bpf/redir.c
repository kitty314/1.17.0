...
SEC("tc_clash.meta_auto_redir_ingress")
...
SEC("tc_clash.meta_auto_redir_egress")
int tc_redir_egress_func(struct __sk_buff *skb) {
    ...
    __u32 key = 0, *redir_ip, *redir_port; // *clash.meta_mark

//    clash.meta_mark = bpf_map_lookup_elem(&redir_params_map, &key);
//    if (clash.meta_mark && *clash.meta_mark != 0 && *clash.meta_mark == skb->mark)
//        return TC_ACT_OK;
